...
const normaliseMenu = (menu) => {
  ...
  if (typeof menu === 'string') {
    return menu.split(',').map((item) => item.trim()).filter(Boolean);
  }
  return [];
};
...
exports.deleteRestaurantByName = asyncHandler(async (req, res) => {
  const name = req.params?.name || req.body?.name;
  if (!name) {
    return res.status(400).json({ error: { message: "'name' is required" } });
  }

  const deleted = await restaurantService.deleteRestaurantByName(name);
  if (!deleted) {
